...
@section('headerWithButton')
    <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2 md:gap-3 lg:gap-4">
        ...
        <!-- Add Button -->
        <x-buttons.form-create-button route="{{ route('classes.create') }}" name="Classes" permission="classes" />
    </div>
@endsection
...
@section('content')
    <table id="classesTable" class="display" width="100%">
        <thead class="bg-gray-50">
            <tr class="{{ $trClass }}">
                <th>S.N</th>
                <th>Name</th>
                <th>Is Published</th>
                <th>Is Admission Allowed</th>
                <th>Actions</th>
            </tr>
        </thead>
...
@section('scripts')
    @include('backend.classes.partials.scripts')
@endsection
