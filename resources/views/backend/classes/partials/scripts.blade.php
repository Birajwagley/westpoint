<script>
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        const editUrl = "{{ route('classes.edit', ':id') }}".replace(':id', data
                            .id);
                        const deleteUrl = "{{ route('classes.destroy', ':id') }}".replace(':id',
                            data.id);

                        return `
                                <div class="flex space-x-2">
                                    <x-buttons.edit-button url="${editUrl}" />
                                    <x-buttons.delete-button url="${deleteUrl}" class="classes-delete"/>
                                </div>
                            `;
                    }
                }
        ...
        // Delete Role
        $(document).on("click", ".classes-delete", function(e) {
            e.preventDefault();
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response === true) {
                                successToast('Classes deleted successfully.');
                                table.ajax.reload();
                            } else {
                                errorToast(
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Classes');
