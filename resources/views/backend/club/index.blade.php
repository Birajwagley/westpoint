...
@section('headerWithButton')
    <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2 md:gap-3 lg:gap-4">
        ...
        <!-- Add Button -->
        <x-buttons.form-create-button route="{{ route('beyond-academic.create') }}" name="Beyond Academic" permission="beyond academic" />
    </div>
@endsection
...
@section('content')
    <table id="clubTable" class="display" width="100%">
        <thead class="bg-gray-50">
            <tr class="{{ $trClass }}">
                <th>S.N</th>
                <th>School Amenity</th>
                <th>Name</th>
                <th>Is Published</th>
                <th>Is Featured</th>
                <th>Actions</th>
...
@section('scripts')
    @include('backend.club.partials.scripts')
@endsection
