...
<script>
    $(document).ready(function() {
        const table = $('#clubTable').DataTable({
            ...
            ajax: {
                url: "{{ route('beyond-academic.data') }}",
                dataSrc: 'datas',
            },
            columns: [{
                    data: null,
                ...
                },
                {
                    ...
                    render: function(data) {
                        const editUrl = "{{ route('beyond-academic.edit', ':id') }}".replace(':id',
                            data
                            .id);
                        const deleteUrl = "{{ route('beyond-academic.destroy', ':id') }}".replace(
                            ':id',
                            data.id);
                        ...
                        return `
                                <div class="flex space-x-2">
                                    ...
                                    <x-buttons.delete-button url="${deleteUrl}" class="club-delete"/>
                                </div>
                            `;
                    }
                }
        ...
        // Delete Role
        $(document).on("click", ".club-delete", function(e) {
            e.preventDefault();
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response === true) {
                                successToast('Club  deleted successfully.');
                                table.ajax.reload();
                            } else {
                                errorToast(
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Club');
