<script>
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        const editUrl = "{{ route('designation.edit', ':id') }}".replace(':id',
                            data
                            .id);
                        const deleteUrl = "{{ route('designation.destroy', ':id') }}".replace(
                            ':id',
                            data.id);
                        ...
                        return `
                            <div class="flex space-x-2">
                                ...
                                ${data.id == 1 || data.id == 2 || data.id == 3 || data.id==4 ? '' :  `<x-buttons.delete-button url="${deleteUrl}" class="publication-category-delete"/>`}
                            </div>
                        `;
                    }
                }
        ...
        // Delete Role
        $(document).on("click", ".designation-delete", function(e) {
            e.preventDefault();
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Designation');
