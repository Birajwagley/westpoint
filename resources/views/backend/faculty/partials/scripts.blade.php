<script>
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        const editUrl = "{{ route('faculty.edit', ':id') }}".replace(':id', data
                            .id);
                        const deleteUrl = "{{ route('faculty.destroy', ':id') }}".replace(':id',
                            data.id);

                        return `
                                <div class="flex space-x-2">
                                    <x-buttons.edit-button url="${editUrl}" />
                                    <x-buttons.delete-button url="${deleteUrl}" class="faculty-delete"/>
                                </div>
                            `;
                    }
                }
        ...
        // Delete Role
        $(document).on("click", ".faculty-delete", function(e) {
            e.preventDefault();
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Faculty');
