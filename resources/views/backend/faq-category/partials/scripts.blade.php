<script>
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        const editUrl = "{{ route('faq-category.edit', ':id') }}".replace(':id',
                            data
                            .id);
                        const deleteUrl = "{{ route('faq-category.destroy', ':id') }}".replace(
                            ':id',
                            data.id);
                        ...
                        return `
                                <div class="flex space-x-2">
                                    ...
                                    <x-buttons.delete-button url="${deleteUrl}" class="faq-category-delete"/>
                                </div>
                            `;
                    }
                }
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response === true) {
                                successToast('Faq Category deleted successfully.');
                                table.ajax.reload();
                            } else {
                                errorToast(
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'FaqCategory');
