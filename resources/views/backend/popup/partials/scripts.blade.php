<script>
    $(document).ready(function() {
        const table = $('#popupTable').DataTable({
            bSortCellsTop: true,
            scrollX: true,
            autoWidth: false,
            responsive: false,
            ...
            columns: [{
                ...
                },
                {
                    ...
                    <img src="${data}" alt="Popup Image" style="height: 50px; width: auto; border-radius: 4px;" /></a>`;
                    }
                },
                {
                    data: 'publish_date'
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data) {
                        const editUrl = "{{ route('popup.edit', ':id') }}".replace(':id', data
                            .id);
                        const deleteUrl = "{{ route('popup.destroy', ':id') }}".replace(':id',
                            data.id);

                        return `
                                <div class="flex space-x-2">
                                    <x-buttons.edit-button url="${editUrl}" />
                                    <x-buttons.delete-button url="${deleteUrl}" class="popup-delete"/>
                                </div>
                            `;
                    }
                }
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response === true) {
                                successToast('Popup deleted successfully.');
                                table.ajax.reload();
                            } else {
                                errorToast(
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Popup');
