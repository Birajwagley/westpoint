<script>
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return row.publication_category ? row.publication_category.name : '';
                    }
                },
                {
                    data: 'slug'
...

                {
                    ...
                    render: function(data) {
                        const editUrl = "{{ route('publication.edit', ':id') }}"
                            .replace(':id', data
                                .id);
                        const deleteUrl = "{{ route('publication.destroy', ':id') }}"
                            .replace(':id',
                                data.id);
                        ...
                        return `
                                <div class="flex space-x-2">
                                    ...
                                    <x-buttons.delete-button url="${deleteUrl}" class="publication-delete"/>
                                </div>
                            `;
                    }
                }
        ...
        // Delete Role
        $(document).on("click", ".publication-delete", function(e) {
            e.preventDefault();
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Publication');
