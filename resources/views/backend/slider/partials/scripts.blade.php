<script>
    $(document).ready(function() {
        const table = $('#sliderTable').DataTable({
            ...
            columns: [{
                ...
                },
                {
                    ...
                    <img src="${data}" alt="Slider Image" style="height: 50px; width: auto; border-radius: 4px;" /></a>`;
                    }
                },
                {
                    data: 'is_published',
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data) {
                        const editUrl = "{{ route('slider.edit', ':id') }}".replace(':id', data
                            .id);
                        const deleteUrl = "{{ route('slider.destroy', ':id') }}".replace(':id',
                            data.id);

                        return `
                                <div class="flex space-x-2">
                                    <x-buttons.edit-button url="${editUrl}" />
                                    <x-buttons.delete-button url="${deleteUrl}" class="slider-delete"/>
                                </div>
                            `;
                    }
                }
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Slider');
