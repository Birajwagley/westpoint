<script>
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        ...
                        return `<i class="${data} fa-2x" target="_blank"></i>`;
                    }
                },
                {
                    data: 'count'
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data) {
                        const editUrl = "{{ route('statistic.edit', ':id') }}".replace(':id',
                            data.id);
                        const deleteUrl = "{{ route('statistic.destroy', ':id') }}".replace(
                            ':id', data.id);

                        return `
                                <div class="flex space-x-2">
                                    <x-buttons.edit-button url="${editUrl}" />
                                    <x-buttons.delete-button url="${deleteUrl}" class="statistic-delete"/>
                                </div>
                            `;
                    }
                }
        ...
        // Delete Role
        $(document).on("click", ".statistic-delete", function(e) {
            e.preventDefault();
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response === true) {
                                successToast('Statistic deleted successfully.');
                                table.ajax.reload();
                            } else {
                                errorToast(
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Statistic');
