...
<script>
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        const editUrl = "{{ route('team.edit', ':id') }}".replace(':id',
                            data
                            .id);
                        const deleteUrl = "{{ route('team.destroy', ':id') }}".replace(
                            ':id',
                            data.id);
                        ...
                        return `
                                <div class="flex space-x-2">
                                    ...
                                    <x-buttons.delete-button url="${deleteUrl}" class="teams-delete"/>
                                </div>
                            `;
                    }
                }
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response === true) {
                                successToast('Teams  deleted successfully.');
                                table.ajax.reload();
                            } else {
                                errorToast(
        ...
        $(document).on("change", ".is-published-toggle", function() {
            ...
            updateUrl = updateUrl.replace(':id', id).replace(':model', 'Teams');
