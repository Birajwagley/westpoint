...
<section class="w-full py-8 relative z-0">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-12">
        ...
        <div id="yourGateway-carousel"
           ...
           w-full max-w-[1070px] mx-auto
           bg-gradient-to-r from-primary to-secondary
           rounded-2xl shadow-xl p-4 sm:p-6 lg:p-4 z-10">
            ...
            <div class="flex slider-container items-center justify-between w-full lg:w-auto space-x-4">
                <span id="wrapper-first"
                    class="flex items-center justify-center w-10 h-10 rounded-full bg-secondary text-white cursor-pointer text-sm sm:text-base">
                    <i class="fa-solid fa-chevron-left"></i>
                </span>

                <div
                    class="flex slides-wrapper scroll-smooth overflow-x-auto no-scrollbar w-full sm:w-[calc(2*128px+1rem)] md:w-[600px]  px-2 gap-4 sm:gap-6 md:gap-10 py-2 rounded-lg">
                    @foreach ($quickNavs as $quickNav)
                        <a href="{{ $quickNav->type == MenuTypeEnum::SLUG->value ? url($quickNav->slug) : $quickNav->external_link }}"
                            id="slideItem"
                            class="w-28 sm:w-1/2 min-w-[128px] h-24 sm:h-28 rounded-[10px] bg-white flex flex-col items-center justify-center space-y-2 p-2">

                            <i class="{{ $quickNav->icon }} fa-2x sm:fa-2x md:fa-3x lg:fa-3x text-secondary"></i>
