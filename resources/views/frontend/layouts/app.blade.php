...
<body>
    ...
    @if (!Route::is('home') && isset($drawerNavigations))
        {{-- drawer navigation --}}
        @include('frontend.layouts.partials.drawer-navigation')
    @endif

    <!-- Content -->
    <div id="content" class="wrapper bg-[#F2F5FA]">
        <!-- FAQ Button -->
        <a href="{{ route('faqs') }}">
            <div
                class="z-50 fixed bottom-16 right-4 cursor-pointer bg-gray-800 px-3 py-2 rounded-md text-white tracking-wider shadow-xl animate-bounce hover:animate-none">
                <svg height="25px" width="24px" version="1.1" id="x32" xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512.00 512.00" xml:space="preserve"
                    fill="#ffffff" stroke="#ffffff" stroke-width="0.00512">
        ...
        <button id="goToTopBtn"
            class="fixed bottom-4 right-4 z-50 p-3 rounded-full bg-yellow-500 text-white shadow-lg transition-all duration-300 hover:scale-110 hover:bg-black hidden"
            aria-label="Scroll to top">
            <img src="{{ asset('assets/frontend/images/homepage/arrow-top.png') }}" alt="Go to top"
